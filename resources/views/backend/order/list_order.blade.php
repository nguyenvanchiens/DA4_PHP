...
<section id="main-content">
  ...
  <div class="panel panel-default">
    <div class="panel-heading">
      DANH SÁCH ĐƠN HÀNG CHƯA XÁC THỰC
    </div>
        ...
        <tbody>
          ...
          @endforeach
            <tr class="text-center">
                ...
                <td>   
                  @if($item->payment==1)
                    <span class="text-primary">Đơn hàng thành công</span>
                  @else
                    @if ($item->status==0)
                ...
                <td>
                  ...
                  @else
                  @if ($item->status==1)
                  <a href="{{ asset('admin/order/succes/'.$item->id) }}" data-url="" class="btn btn-sm btn-primary action_delete"><i class="fas fa-check-circle"></i>Xong</a>
                  ...
                  @else
                  <a href="{{ asset('admin/order/vieworder/'.$item->id) }}" class="btn btn-sm btn-warning"><i class="fas fa-eye"></i> View</a>
                    <a href="{{ asset('admin/order/orderprocessing/'.$item->id) }}" class="btn btn-sm btn-primary"><i class="fas fa-check-square"></i> Xử Lý</a>
                    <a href="{{ asset('admin/order/DeleteOrder/'.$item->id) }}" data-url="" class="btn btn-sm btn-danger action_delete"><i style="color:white" class="glyphicon glyphicon-trash"></i> Xóa</a>
                  @endif
                  @endif
                  </td>
